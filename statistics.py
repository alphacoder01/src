...
class Statistics:
    def __init__(self, court_tracker: CourtDetector, players_detection: DetectionModel):
        ...
        self.feet_bottom, self.feet_top = self.players_detection.calculate_feet_positions(
            self.court_tracker)
    ...
    def get_player_position_heatmap(self, pit_size=80):
        ...
        for x, y in self.feet_bottom:
            x = int(x)
            y = int(y)
            heatmap[min(heatmap_shape[0] - 1, y // pit_size),
                    min(heatmap_shape[1] - 1, x // pit_size)] += 1
    ...
    def get_players_dists(self):
        ...
        self.display_heatmap(
            heatmap, self.court_tracker.court_reference.court, cmap=None, title='Players path')
